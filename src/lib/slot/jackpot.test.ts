...
describe("duel", () => {
  it("hot-seat 10 spins, higher score wins, tie is remíza", () => {
    ...
    assert.equal(canDuelSpin(ahead), true);
    assert.equal(duelView(ahead).waiting, false);
    assert.equal(duelView(ahead).k, 1);
    assert.equal(duelView(ahead).mine, 5);
    ...
    let finished = caught;
    for (let i = 0; i < 9; i++) finished = tickDuel(finished, 1);
    assert.equal(canDuelSpin(finished), false);
    assert.equal(duelView(finished).waiting, true);
